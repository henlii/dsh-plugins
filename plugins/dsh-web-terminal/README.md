...
一对多、命令执行结束通知使用它的会话。Web 面板在**输入框下方**，REPL 式直接输入，
tab 标签切换多个终端。
...
- 位置：**输入框下方**（`conversation.composer.dock`），完全贴边、两端对齐、无圆角卡片
  （运行时抵消容器 padding）。
- **REPL 式**：点击终端直接输入，Enter 执行，Ctrl+C 中断——**没有独立的输入框和
  发送/中断/强杀按钮**；错误信息显示在输出区顶部。
- **tab 标签**：折叠按钮（▾）在最前，随后是各终端 tab（本会话带 ★）、`+` 新建；点击
  `×` 关闭。
- **拖动调高度**：终端顶部**分隔线本身**就是拖动杆（中间有小圆角方块标识，与右侧栏
  分隔线风格一致），上下拖动调整输出高度（60–600px）。
- 保留输入框底部的官方 stats 统计行。
